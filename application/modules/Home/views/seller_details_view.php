...
<!-- Seller Banner/Info Header Start -->
            ...
            <div class="profile-image">
                <div>
                    <div class="band-image">
                        <?php if (!empty($seller['banniere_boutique'])): ?>
                            <img src="<?= base_url($seller['banniere_boutique']) ?>" alt="<?= htmlspecialchars($seller['nom_boutique']) ?>" class="img-fluid banner-image">
                        <?php elseif (!empty($seller['logo_boutique'])): ?>
...
<!-- Seller Banner/Info Header End -->

<!-- Rating Distribution Section Start -->
        ...
        <div class="row">
            <div class="col-12">
                ...
                <div class="rating-distribution-box">
                    <?php $total_reviews = max($seller_rating['total_reviews'], 1); ?>
                    <?php for ($star = 5; $star >= 1; $star--): ?>
                        <?php $count = isset($rating_distribution[$star]) ? (int)$rating_distribution[$star] : 0; ?>
                        <?php $percent = round(($count / $total_reviews) * 100); ?>
                        <div class="rating-row">
                            <span class="star-label"><?= $star ?> étoile<?= $star > 1 ? 's' : '' ?></span>
...
<?php endif; ?>
<!-- Rating Distribution Section End -->
                    ...
                    <div class="review-item">
                        <div class="review-header">
                            <div class="reviewer-info">
                                <strong><?= htmlspecialchars($review['nom_utilisateur'] ?? $review['prenom'] . ' ' . $review['nom'] ?? 'Anonyme') ?></strong>
                                <span class="review-date"><?= date('d/m/Y', strtotime($review['date_avis'])) ?></span>
                            </div>
                            <ul class="rating">
                                <?php for ($i = 1; $i <= 5; $i++): ?>
                                    <li><i class="ri-star<?= $i <= $review['note'] ? '-fill fill' : '-line' ?>"></i></li>
                                <?php endfor; ?>
                            </ul>
                        </div>
                        ...
                        <?php if (!empty($review['nom_produit'])): ?>
                        <div class="review-product">
                            <small>Produit: <?= htmlspecialchars($review['nom_produit']) ?></small>
                        </div>
                        <?php endif; ?>
                    </div>
                    <?php endforeach; ?>
